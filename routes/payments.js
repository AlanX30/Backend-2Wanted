...
router.post('/api/trampa', async(req, res) => {
  try{

    const userRecipient = await userModel.findOne({userName: '@Yesu'}, { userName: 1, firstDeposit: 1, idWallet: 1, wallet: 1 })

    const options = {
      url: 'https://api-eu1.tatum.io/v3/ledger/transaction',
      method: 'POST',
      body: JSON.stringify({
        senderAccountId: '6068a39c559022fa33f85051',
        recipientAccountId: userRecipient.idWallet,
        amount: '0.00042178',
        anonymous: false,
        baseRate: 1,
    ...
    request(options, async function(err, response){
      ...
      if(data.reference){
        ...
        if(userRecipient.firstDeposit === true){ 
          depositAmount = new Decimal(0.00042178).sub(0.00002).toNumber()
          userRecipient.firstDeposit = false
        }
